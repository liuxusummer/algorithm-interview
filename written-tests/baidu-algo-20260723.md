...
title: 百度算法岗 2026-07-23
description: 百度 2026-07-23 算法岗笔试真题，含 3 道 Python ACM 模式题解
---

<div class="exam-session-banner">
  <div>
    <span>BAIDU / 2026.07.23 / ACM</span>
    <strong>百度 · 算法岗</strong>
    <small>2026-07-23 · 3 题 ACM · 难度 中等偏难</small>
  </div>
  <div class="exam-session-banner__meta">
    <span>相邻翻转与被迫决策贪心</span>
    <span>前缀最值与线性动态规划</span>
    <span>质数筛、动态颜色维护与增量路径</span>
  </div>
</div>

# 百度 2026-07-23 算法岗笔试解析

本场统一整理为完整 Python 3 ACM 程序。每题保留标准输入输出、建模过程、正确性依据、复杂度、易错点和关键中文注释；原 SQL、Shell 与第三方机器学习库题也已改写为可独立运行的标准库版本。

来源：[Zero2Leetcode · 百度 2026-07-23 算法岗](https://onefly.top/zero2Leetcode/04_real_interviews/baidu/algo-20260723/)。
...
```python
# 核心步骤：相邻翻转与被迫决策贪心；严格按题面处理边界并输出结果。
import sys
...
```python
# 核心步骤：前缀最值与线性动态规划；严格按题面处理边界并输出结果。
import sys
input = sys.stdin.readline
...
```python
# 核心步骤：质数筛、动态颜色维护与增量路径；严格按题面处理边界并输出结果。
import sys
input = sys.stdin.readline
