...
title: 携程算法岗 2026-05-21
description: 携程 2026-05-21 算法岗笔试真题，含 4 道 Python ACM 模式题解
---

<div class="exam-session-banner">
  <div>
    <span>CTRIP / 2026.05.21 / ACM</span>
    <strong>携程 · 算法岗</strong>
    <small>2026-05-21 · 4 题 ACM · 难度 中等</small>
  </div>
  <div class="exam-session-banner__meta">
    <span>模拟</span>
    <span>三维网格枚举</span>
    <span>sklearn GridSearchCV + StandardScaler</span>
    <span>中途相遇法 + 哈希计数</span>
  </div>
</div>

# 携程 2026-05-21 算法岗笔试解析

本场统一整理为完整 Python 3 ACM 程序。每题保留标准输入输出、建模过程、正确性依据、复杂度、易错点和关键中文注释；原 SQL、Shell 与第三方机器学习库题也已改写为可独立运行的标准库版本。

来源：[Zero2Leetcode · 携程 2026-05-21 算法岗](https://onefly.top/zero2Leetcode/04_real_interviews/ctrip/algo-20260521/)。
...
1. 第一题：模拟（难度简单）
2. 第二题：三维网格枚举（难度中等）
3. 第三题：纯 Python GridSearchCV + StandardScaler（难度中等）
...
- 第三题熟悉 纯 Python 管线：StandardScaler 只在训练集 fit、StratifiedKFold 保比例、GridSearchCV 字典序选优
...
## 03 · SVM网格搜索 {#problem-03}

### 题目描述

请在仅依赖 Python 标准库 / pandas / 纯 Python 的前提下，实现一个线性支持向量机（SVM）分类器，并通过 Stratified 3-Fold 交叉验证 + 网格搜索自动选择最佳惩罚系数 $C$。完成后对给定测试样本输出类别预测。
...
- 评估指标：`accuracy`。
- 手写组合以上步骤；当多组参数获得相同最高分时，GridSearchCV 会按参数字典序选择最先出现的那组（即较小的 $C$ 值）。
...
先只用训练折统计每一维的均值和标准差，再转换训练折、验证折与最终测试集，避免验证信息泄漏。分类器使用线性软间隔 SVM 的原始形式：权重承担 $L2$ 正则；当样本间隔小于 1 时，再叠加 hinge loss 的次梯度。
...
- 第三题是 纯 Python 管线题，严格按题目参数搭建 StandardScaler + SVC + StratifiedKFold + GridSearchCV 管线即可
