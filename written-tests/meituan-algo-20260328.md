...
title: 美团算法岗 2026-03-28
description: 美团 2026-03-28 算法岗笔试真题，含 4 道 Python ACM 模式题解
---

<div class="exam-session-banner">
  <div>
    <span>MEITUAN / 2026.03.28 / ACM</span>
    <strong>美团 · 算法岗</strong>
    <small>2026-03-28 · 4 题 ACM · 难度 中等偏难</small>
  </div>
...
# 美团 2026-03-28 算法岗笔试解析

本场题目按原考试输入输出整理为完整 Python 程序。每题依次说明建模依据、状态或数据结构、正确性理由、边界处理与复杂度。

来源：[Zero2Leetcode · 美团 2026-03-28 算法岗](https://onefly.top/zero2Leetcode/04_real_interviews/meituan/algo-20260328/)。
...
**解释**：共有 8 个区间满足条件。例如相邻两个正整数构成的区间一定合法，因为区间最大值必然严格小于两个端点之和。
...
2. 对区间 [L, R] 分治：
   - 找到最大值位置 m
   - 递归处理 [L, m-1] 和 [m+1, R]
   - 先统计以 m 为端点的区间，再统计严格跨越 m 的区间（l < m < r）

3. 以 m 为端点时，另一个端点为正数，所以区间必然合法；严格跨越 m 时，条件简化为 a[m] < a[l] + a[r]，即 a[l] > a[m] - a[r] 或 a[r] > a[m] - a[l]
...
def main():
    ...
    # LOG[x] = floor(log2(x))，供 RMQ 查询使用。
    log = [0] * (n + 1)
    for i in range(2, n + 1):
        log[i] = log[i // 2] + 1

    levels = log[n] + 1
    sp = [[0] * n for _ in range(levels)]
    for i in range(n):
        sp[0][i] = i
    for level in range(1, levels):
        ...
        for left in range(n - length + 1):
            x = sp[level - 1][left]
            y = sp[level - 1][left + half]
            sp[level][left] = x if a[x] >= a[y] else y

    def query_max_pos(l, r):
        """返回 a[l..r] 中最大值的位置"""
        level = log[r - l + 1]
        x = sp[level][l]
        y = sp[level][r - (1 << level) + 1]
    ...
    while stack:
        ...
        mid = query_max_pos(left, right)

        # 最大值作为一个端点时，另一端为正数，因此这些区间全部合法。
        ans += (mid - left) + (right - mid)

        # 两个端点分居最大值两侧时，枚举短侧并在长侧二分。
        if mid - left <= right - mid:
            right_values = sorted(a[mid + 1:right + 1])
            for i in range(left, mid):
                first = bisect_right(right_values, a[mid] - a[i])
                ans += len(right_values) - first
        else:
            left_values = sorted(a[left:mid])
            for j in range(mid + 1, right + 1):
                first = bisect_right(left_values, a[mid] - a[j])
                ans += len(left_values) - first

        # 用显式栈避免单调数组导致 Python 递归过深。
