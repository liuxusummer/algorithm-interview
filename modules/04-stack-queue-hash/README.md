...
[单调栈与单调队列系统详解](./monotonic-stack-queue)从支配关系和不变量出发，系统讲解最近更值、左右边界、重复值归属、滑动窗口最值与单调队列优化 DP。
...
| [原创 01 · 带过期时间的 LRU 缓存](/problems/original-001-expiring-lru-cache) | 双向链表 + 可选最小堆 | 独立 TTL 用堆，固定续期 TTL 可从尾部清理 |
...
- [单调栈与单调队列](./monotonic-stack-queue)：每日温度、柱状图最大矩形、滑动窗口最值与 DP 优化；
- 表达式求值与逆波兰表达式；
- LFU 缓存与频次链表；
- Top K 数据流与可删除堆；
- 双端队列在 BFS 和窗口问题中的应用。
